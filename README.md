...
Durante el desarrollo aparecieron errores no triviales. El caso más representativo fue una **cadena de errores de tipos estrictos que solo afloraban en el build de producción de Vercel** y no en el entorno de desarrollo local. Usé la IA como apoyo para acelerar el diagnóstico —leer trazas, descartar hipótesis y ubicar la causa raíz más rápido—, pero la decisión de cómo resolver de fondo fue criterio propio de QA: tipar correctamente en lugar de silenciar el error con `any` o `@ts-ignore`. La regla fue siempre resolver la causa, no parchar el síntoma.
...
Para mantener un historial de Git limpio y profesional, delegué la redacción de los mensajes de commit a Google Gemini. Lo elegí puntualmente para esta tarea por ser de alto volumen y bajo riesgo, fácil de separar del flujo principal de codificación con Claude: así pude **commitear seguido y de forma atómica sin frenar el desarrollo para redactar cada mensaje a mano**. El resultado es un historial de commits atómicos, cada uno con un mensaje claro y descriptivo que refleja un cambio acotado.
